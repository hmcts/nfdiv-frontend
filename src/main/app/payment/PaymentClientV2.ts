...
export class PaymentClientV2 {
  ...
  constructor(
    ...
    logger.info(session.user.accessToken);
    logger.info(getServiceAuthToken());
  }
  ...
  public async create(serviceRequestNumber: string): Promise<Payment> {
    ...
    try {
      logger.info('Will try to take the payment now using the service request reference number: Request body is : ');
      logger.info(bodyCardPay);
