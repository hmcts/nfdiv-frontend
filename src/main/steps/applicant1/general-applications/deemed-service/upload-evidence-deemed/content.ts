...
const en = ({ partner, userCase }: CommonContent, applicant1UploadDocumentContent) => ({
  title: 'Upload your evidence',
  line1: `Upload your evidence to support your application for deemed service. This should be evidence you've found from after the date that your application was issued: ${userCase.issueDate}`,
  line2:
...
const cy: typeof en = ({ partner, userCase }: CommonContent, applicant1UploadDocumentContent) => ({
  title: 'Upload your evidence',
  line1: `Upload your evidence to support your application for deemed service. This should be evidence you've found from after the date that your application was issued: ${userCase.issueDate}`,
  line2:
    "If you're uploading images or screenshots of a recent conversation by text, email or social media, make sure they include:",
  ...
  fields: userCase => ({
    applicant1DeemedEvidenceUploadedFiles: {
      ...
      validator: (value, formData) => {
        ...
        const selectedCannotUploadDocuments = !!formData.applicant1DeemedCannotUploadDocs?.length;
    ...
    applicant1DeemedCannotUploadDocs: {
      ...
      validator: (value, formData) => {
        const hasUploadedFiles =
          (formData.applicant1DeemedEvidenceUploadedFiles as unknown as string[])?.length &&
          (formData.applicant1DeemedEvidenceUploadedFiles as unknown as string) !== '[]';
        const selectedCannotUploadDocuments = !!formData.applicant1DeemedCannotUploadDocs?.length;
        if (!hasUploadedFiles && !selectedCannotUploadDocuments) {
          return 'notUploaded';
        }
      ...
      values: [
        {
          name: 'applicant1DeemedCannotUploadDocs',
          label: l => l.cannotUpload,
