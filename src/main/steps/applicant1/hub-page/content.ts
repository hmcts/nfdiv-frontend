...
import { StateSequence, currentStateFn } from '../../state-sequence';
...
const en = ({ isDivorce, userCase, referenceNumber, partner, isJointApplication, isApplicant2 }: CommonContent) => {
  ...
  return {
    ...
    response: accessibleProgressBarSpan('Response', currentState.isAfter('IssuedToBailiff')),
    conditionalOrderApplication: accessibleProgressBarSpan(
      'Conditional order application',
      currentState.isAfter('AwaitingLegalAdvisorReferral')
    ),
    conditionalOrderGranted: accessibleProgressBarSpan(
      'Conditional order granted',
      currentState.isAtOrAfter('ConditionalOrderPronounced')
    ),
    finalOrderApplication: accessibleProgressBarSpan(
      'Final order application',
      currentState.isAfter('FinalOrderRequested')
    ),
    applicationEnded: accessibleProgressBarSpan(
      isDivorce ? 'Divorced' : 'Civil partnership ended',
      userCase.state === 'FinalOrderComplete'
    ),
