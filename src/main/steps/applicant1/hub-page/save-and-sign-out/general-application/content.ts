...
import { findUnpaidGeneralApplication, getGeneralApplicationServiceRequest } from '../../../../../app/utils/general-application-utils';
...
const en = ({ isDivorce }: CommonContent, hasPaymentInProgress) => ({
  generalApplication: {
    ...
    line2: 'You can continue and complete the application or withdraw the application if you no longer want to continue.',
    ...
    continueLinkText: 'Complete payment',
...
export const generateContent: TranslationFn = content => {
  ...
  const hasGeneralApplicationPaymentInProgress = !!findUnpaidGeneralApplication(
    userCase, getGeneralApplicationServiceRequest(content.isApplicant2, userCase) as string
  );
  const showGeneralApplicationSaveAndSignOutUpdate = interimApplicationType === InterimApplicationType.DIGITISED_GENERAL_APPLICATION_D11
