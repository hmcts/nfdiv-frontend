...
describe('SoleTemplateChooser test', () => {
  ...
  test('redirects to the /final-order-requested.njk for state FinalOrderRequested', () => {
  ...
  test('redirects to the /awaiting-service-consideration-or-awaiting-bailiff-referral.njk for state AwaitingServiceConsideration', () => {
  ...
  test('redirects to the /awaiting-service-consideration-or-awaiting-bailiff-referral.njk for state AwaitingBailiffReferral', () => {
    const theState = displayState.at(State.AwaitingBailiffReferral);
    const soleTemplate = getSoleHubTemplate(theState, false, false, false);
    expect(soleTemplate).toBe('/awaiting-service-consideration-or-awaiting-bailiff-referral.njk');
  });

  test('redirects to the /conditional-order-pronounced.njk for state ConditionalOrderPronounced', () => {
    const theState = displayState.at(State.ConditionalOrderPronounced);
    const soleTemplate = getSoleHubTemplate(theState, false, false, false);
    expect(soleTemplate).toBe('/conditional-order-pronounced.njk');
  });

  test('redirects to the /awaiting-legal-advisor-referral-or-awaiting-pronouncement.njk for state AwaitingLegalAdvisorReferral', () => {
  ...
  test('redirects to the /awaiting-legal-advisor-referral-or-awaiting-pronouncement.njk for state AwaitingPronouncement', () => {
    const theState = displayState.at(State.AwaitingPronouncement);
    const soleTemplate = getSoleHubTemplate(theState, false, false, false);
    expect(soleTemplate).toBe('/awaiting-legal-advisor-referral-or-awaiting-pronouncement.njk');
  });

  test('redirects to the /awaiting-general-consideration.njk for state AwaitingGeneralConsideration', () => {
  ...
  test('redirects to the /awaiting-bailiff-service.njk for state FinalOrderOverdue', () => {
    const theState = displayState.at(State.FinalOrderOverdue);
    const soleTemplate = getSoleHubTemplate(theState, false, false, false);
    expect(soleTemplate).toBe('/awaiting-final-order-or-final-order-overdue.njk');
  });

  test('redirects to the /bailiff-service-unsuccessful.njk for state AwaitingAos and isServiceApplicationGranted and not isSuccessfullyServedByBailiff', () => {
  ...
  test('redirects to the /service-application-rejected.njk for state AwaitingAos and isAlternativeService and not isServiceApplicationGranted', () => {
    const theState = displayState.at(State.AwaitingAos);
    const soleTemplate = getSoleHubTemplate(theState, false, false, true);
    expect(soleTemplate).toBe('/service-application-rejected.njk');
  });

  test('redirects to the /aos-awaiting-or-drafted.njk for state AwaitingAos', () => {
    const theState = displayState.at(State.AwaitingAos);
    const soleTemplate = getSoleHubTemplate(theState, false, false, false);
    expect(soleTemplate).toBe('/aos-awaiting-or-drafted.njk');
  });

  test('redirects to the /aos-due.njk for states after AosDrafted and before Holding', () => {
    const theState = displayState.at(State.AosOverdue);
    const soleTemplate = getSoleHubTemplate(theState, false, false, false);
    expect(soleTemplate).toBe('/aos-due.njk');
  });

  test('should default to /aos-awaiting-or-drafted.njk', () => {
