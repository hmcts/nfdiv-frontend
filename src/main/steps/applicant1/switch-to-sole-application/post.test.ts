...
import { ApplicationType, DivorceOrDissolution, State, WITHDRAW_JOINT_APPLICATION } from '../../../app/case/definition';
import { Form } from '../../../app/form/Form';
import { HOME_URL, PAY_AND_SUBMIT_URL, YOUR_DETAILS_URL } from '../../urls';
...
describe('SwitchToSoleApplicationPostController', () => {
  ...
  test('Should redirect to pay and submit page when cancel button used in Applicant2Approved state', async () => {
    ...
    expect(res.redirect).toBeCalledWith(PAY_AND_SUBMIT_URL);
    expect(req.session.errors).toStrictEqual([]);
  });
  ...
  test('Should return error when new case could not be created and redirect to the same page', async () => {
    const errors = [] as never[];
    const body = {};
    const mockForm = {
      setFormState: jest.fn(),
