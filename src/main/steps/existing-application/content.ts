...
const en = ({ isDivorce, partner, isJointApplication, required }) => {
  const respondJoin = `${isJointApplication ? 'respond to' : 'join'}`;
  return {
    title: 'You have an existing application',
    line1: `You already have an existing application for ${isDivorce ? 'divorce' : 'to end your civil partnership'}`,
