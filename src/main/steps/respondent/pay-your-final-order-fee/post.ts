...
import { CITIZEN_ADD_PAYMENT, RESPONDENT_APPLY_FOR_FINAL_ORDER, PaymentStatus, State } from '../../../app/case/definition';
import { AppRequest } from '../../../app/controller/AppRequest';
import { AnyObject } from '../../../app/controller/PostController';
import { PaymentClient, Payment } from '../../../app/payment/PaymentClient';
...
@autobind
export default class FinalOrderPaymentPostController {
  public async post(req: AppRequest<AnyObject>, res: Response): Promise<void> {
    if (req.body.saveAndSignOut || req.body.saveBeforeSessionTimeout) {
      return res.redirect(SAVE_AND_SIGN_OUT);
    }

    if (req.session.userCase.state !== State.AwaitingRespondentFOPayment) {
      req.session.userCase = await req.locals.api.triggerEvent(req.session.userCase.id, {}, RESPONDENT_APPLY_FOR_FINAL_ORDER);
    }

    const payments = new PaymentModel(req.session.userCase.finalOrderPayments || []);
