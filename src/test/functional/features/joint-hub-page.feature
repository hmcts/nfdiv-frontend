Feature: Joint hub page
  ...
  Scenario: Hub applicant 1 confirms receipt
  ...
  Scenario: Hub applicant 2 confirms receipt
    And I set the case state to "Holding"
    And I go to "/"
  ...
  Scenario: Hub applicant 1 draft conditional order from AwaitingConditionalOrder state
  ...
  Scenario: Hub applicant 2 draft conditional order from AwaitingConditionalOrder state
    And I set the case state to "AwaitingConditionalOrder"
    When I go to "/"
    Then the page should include "You can now apply for a ‘conditional order’"
    When I click "Apply for conditional order"
